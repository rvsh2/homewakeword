...
def _run_replay(
    ...
    *,
    ...
    exit_code = main([
        ...
        '--wake-word', wake_word,
        '--input', str(FIXTURE_ROOT / 'stream' / input_name),
        '--expect', expect,
...
def test_runtime_advertises_only_validated_v1_pack_entries() -> None:
    ...
    assert [wake_word.name for wake_word in service.server.describe().wake_words] == list(EXPECTED_V1_WAKE_WORDS)
    assert len(service.inventory) == len(EXPECTED_V1_WAKE_WORDS)

    inventory = {record.wake_word: record.as_report_dict() for record in service.inventory}
