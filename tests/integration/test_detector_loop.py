...
def _run_replay(
    tmp_path: Path, *, manifest: str, input_name: str, expect: str
) -> DetectorReplayPayload:
    output_path = tmp_path / f"{Path(input_name).stem}.json"
    exit_code = main(
        [
            "--manifest",
            str(MANIFEST_ROOT / manifest),
            "--input",
            str(FIXTURE_ROOT / input_name),
            "--expect",
            expect,
            "--json-out",
            str(output_path),
...
def test_positive_replay_emits_exactly_one_detection(tmp_path: Path) -> None:
    ...
    assert payload["detector_counters"]["vad_suppressions"] == 0
