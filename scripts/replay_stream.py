...
def _detector_payload(
    ...
    return 0, {
        ...
        "expected": args.expect,
        "wake_word": manifest.wake_word,
        "detection": detections[0].label
        if len(detections) == 1
        else ("none" if not detections else "multiple"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if manifest.mode == "frontend_only":
        ...
        if args.expect == "none" and frontend_payload["detection"] != "none":
            print(
                "frontend-only replay unexpectedly reported a detection",
                file=sys.stderr,
            )
            return 1
        ...
        if args.expect != "none" and (
            ...
            print(
                "detector replay did not emit exactly one expected detection",
                file=sys.stderr,
            )
            return 1
