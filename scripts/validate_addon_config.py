...
def _validate_against_schema(config: dict[str, Any], options: dict[str, Any]) -> None:
    ...
    if not isinstance(manifest, str) or not manifest.strip():
        raise AddonConfigValidationError("option 'manifest' must be a non-empty string")
...
def validate_addon_config(config_path: Path, options_path: Path) -> str:
    ...
    return (
        ...
        f"slug={config['slug']} host={options['host']} port={options['port']} backend={options['detector_backend']}"
    )
