...
class StreamingDetectionStateMachine:
    ...
    _vad_suppressions: int = field(default=0, init=False)
    ...
    def counters(self) -> DetectorLoopCounters:
        return DetectorLoopCounters(
            ...
            vad_suppressions=self._vad_suppressions,
    ...
    def record_vad_suppression(self) -> None:
        self._vad_suppressions += 1
    ...
    def evaluate(
        self, *, score: float, threshold: float, frame_duration_seconds: float
    ) -> tuple[bool, DetectorRuntimeState]:
        ...
        detected = (
            self._consecutive_hits >= self.required_consecutive_hits
        ...
        if detected:
            ...
            refractory_remaining = max(
                0.0, self._refractory_until_seconds - now_seconds
            )
